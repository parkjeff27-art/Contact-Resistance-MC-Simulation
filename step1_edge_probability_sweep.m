%% Graphene-Metal Interface: STEP1 Sweep (REALTIME) - Fixed Width Geometry
%  Target: Threadripper PRO + RTX 5090 (but runs on CPU if GPU not found)
...
%  STEP1 Sweep: P_edge = [0.6 0.7 0.8 0.9], P_surface fixed = 0.015
...
% IMPORTANT: This is a 2-column cell array. Use CONFIGS{k,1}, CONFIGS{k,2}.
CONFIGS = {
...
P_SURFACE      = 0.015;     % fixed for STEP1
P_EDGE_LIST    = [0.6 0.7 0.8 0.9];

DIFFUSION_COEF = 0.5;       % diffusion strength (arbitrary units)
DRIFT_VELOCITY = 0.2;       % +y drift
MAX_STEPS      = 5000;      % enough for TECD
...
for pEdge = P_EDGE_LIST
    ...
    for k = 1:size(CONFIGS,1)
        ...
        fprintf('\n=================================================\n');
        fprintf('▶ STEP1 REALTIME RUN %d/%d\n', run_idx, numel(P_EDGE_LIST)*size(CONFIGS,1));
        fprintf('   %s\n', name);
        fprintf('   Geometry: W=%d um (Fixed) x L=%d um\n', W_um, L_um);
        fprintf('   P_edge=%.3f | P_surface=%.4f (fixed)\n', pEdge, P_SURFACE);

        %% ---- Geometry placement (same spirit as your original) ----
        grid_x_max  = W_um * 1.5;                 % small side margins
        ...
        if USE_GPU
            ...
            active = true(N_PARTICLES, 1, 'gpuArray');   % still alive
            status = zeros(N_PARTICLES, 1, 'gpuArray');  % 0=alive, 1=edge, 2=surf
        else
        ...
        f = figure('Name', sprintf('%s | Pedge=%.2f', name, pEdge), ...
        ...
        if L_um > 100
            daspect(ax1, [1 10 1]);  % compress y for long TECD
        else
            axis(ax1, 'equal');
        end

        rectangle(ax1, 'Position', [pad_x_start, 0, W_um, L_um], ...
            'EdgeColor', 'r', 'LineWidth', 2, 'LineStyle', '--'); % contact rectangle
        rectangle(ax1, 'Position', [pad_x_start, 0, W_um, EDGE_DEPTH_UM], ...
            'FaceColor', 'y', 'EdgeColor', 'none', 'FaceAlpha', 0.5); % edge strip

        % sample indices (about 20k points)
        ...
        % Panel 2: log heatmap (ONLINE accumulation, no transferred_x/y arrays)
        ax2 = subplot(1,3,2); box on;
        title(ax2, 'Transfer Density (Log Scale)');
        xlabel(ax2, 'Width (\mum)'); ylabel(ax2, 'Length (\mum)');

        x_edges = linspace(pad_x_start, pad_x_end, 21); % 20 bins in width
        y_edges = linspace(0, L_um, 101);               % 100 bins in length
        H = zeros(length(y_edges)-1, length(x_edges)-1, 'double'); % CPU histogram
        ...
        h_bar.CData = [1 0.3 0.3; 0.3 0.3 1]; % red/blue
        grid(ax3, 'on');
        set(ax3, 'XTickLabel', {'Edge', 'Surface'});
        ylabel(ax3, 'Count');
        title(ax3, sprintf('Pedge=%.2f | Psurf=%.4f', pEdge, P_SURFACE));
        ...
        for t = 1:MAX_STEPS
            ...
            if any_in
                ...
                trans_edge = is_edge & (r < pEdge);
                trans_surf = is_surf & (r < P_SURFACE);
                ...
                % ONLINE histogram accumulation for new transfers
                if USE_GPU
        ...
        final_stats(run_idx).name = sprintf('%s | Pedge=%.2f', name, pEdge);
        final_stats(run_idx).case = name;
        final_stats(run_idx).P_edge = pEdge;
        final_stats(run_idx).P_surf = P_SURFACE;
...
figure('Name', 'STEP1 Final Comparison (Fixed Width)', 'Color', 'w', 'Position', [200, 200, 1300, 520]);
...
title(sprintf('Surface Ratio (Psurf=%.4f fixed)', P_SURFACE));
ylim([0 100]); grid on;

fprintf('\n=== STEP1 FINAL VERIFICATION ===\n');
